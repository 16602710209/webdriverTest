...
class GuiBase(object):
    ...
    def __init__(self):
        ...
        self.interval = 0.0      #每次点击间隔时间；0为立即执行
    ...
    def click_picture(self, el,clicks=1, button='left'):
        '''点击图片方法'''
        try:
            contrastPath = os.path.join(rootPath, config['pic']['contrastPath'], el)#C:\Users\19344\Desktop\自动化教学\TestDemo\pic\contrast\login_ok.png
            pos_x_y = pyautogui.locateOnScreen(contrastPath, minSearchTime=self.minSearchTime)
            assert pos_x_y
            pyautogui.click(pyautogui.center(pos_x_y),clicks=clicks, button=button,duration=self.duration,interval=self.interval)
    ...
    def screenshot(self, name):
        ...
        logger.info('{}屏幕截图完成'.format(name))
